...
function FilingsTrend({ locale }: { locale: string }) {
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Demandas de Desalojo Anuales en Denver'
          : 'Annual Denver Eviction Filings'}
      </ChartTitle>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ left: 5, right: 10, top: 10, bottom: 5 }}>
          ...
          <ReferenceLine
            ...
            strokeDasharray="4 4"
            label={{
              value: locale === 'es' ? 'Promedio pre-pandemia' : 'Pre-pandemic avg',
              position: 'right',
              fontSize: 11,
              fill: C.muted,
            }}
        ...
        >
          ...
          <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={28}>
            {data.map((entry, i) => (
              <Cell
                key={i}
                fill={i === 0 ? C.danger : entry.isEstimate ? C.accent : C.secondary}
                opacity={entry.isEstimate ? 0.7 : 1}
              />
            ))}
            <LabelList
              dataKey="label"
...
function CourtOutcomesChart({ locale }: { locale: string }) {
  ...
  const answerPct2025 = (
    (sb24064Data.defendantAnswerFiled[2025].yes /
      (sb24064Data.defendantAnswerFiled[2025].yes + sb24064Data.defendantAnswerFiled[2025].no)) *
    100
  ).toFixed(1);
  ...
  const cards = [
    ...
    },
    {
      ...
      label:
        locale === 'es'
          ? 'de inquilinos presentaron respuesta en 2025'
          : 'of tenants filed an answer in 2025',
      sub:
        locale === 'es'
  ...
  return (
    <div>
      <ChartTitle>{locale === 'es' ? 'Qué Pasa en la Corte' : 'What Happens in Court'}</ChartTitle>
      <div
        ...
        {cards.map((c, i) => (
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: '0.8rem', color: C.textMuted, marginTop: '0.2rem' }}>
                {c.sub}
              </div>
            </div>
          </div>
            ...
            {rentalAssistanceBudget.map((entry, i) => (
              <Cell
                ...
                fill={
                  i === 0
                    ? C.primary
                    : i === rentalAssistanceBudget.length - 1
                      ? C.danger
                      : C.accent
                }
              />
...
function HousingContext({ locale }: { locale: string }) {
  const stats = [
    ...
    {
      value: '7.6%',
      label: locale === 'es' ? 'Tasa de vacantes' : 'Vacancy rate',
      color: C.primary,
    },
    {
      value: '52%',
      label: locale === 'es' ? 'Hogares inquilinos' : 'Renter households',
      color: C.secondary,
    },
    {
      value: '$0',
      label: locale === 'es' ? 'Estabilización de alquiler en CO' : 'Rent stabilization in CO',
      color: C.danger,
    },
  ];
        ...
        {stats.map((s, i) => (
          <div
            ...
            style={{
              ...
              background: C.bgAlt,
              borderRadius: 12,
              border: `2px solid ${i === 3 ? C.danger : C.border}`,
            }}
          >
            <div
              style={{
  ...
  const stats = [
    {
      ...
      label:
        locale === 'es'
          ? 'ciudades con menos desalojos en 2025'
          : 'cities saw fewer filings in 2025',
      sub: '',
      color: C.primary,
    },
    {
      value: locale === 'es' ? 'Denver sube' : 'Denver rising',
      label: locale === 'es' ? `Con Austin, Las Vegas, Phoenix` : `With Austin, Las Vegas, Phoenix`,
      sub: '',
      color: C.danger,
    },
    {
      ...
      label:
        locale === 'es' ? 'del caso a la calle en Colorado' : 'from filing to eviction in Colorado',
      sub:
        locale === 'es'
