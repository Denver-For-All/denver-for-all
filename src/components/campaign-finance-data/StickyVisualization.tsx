...
function MoneyExplosion({ locale }: { locale: string }) {
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Gasto Externo en Elecciones de Denver'
          : 'Outside Spending in Denver Elections'}
      </ChartTitle>
...
function WhoPays({ locale }: { locale: string }) {
  const donors = [
    ...
    {
      name: locale === 'es' ? 'Industria Inmobiliaria' : 'Real Estate Industry',
      amount: 578,
      label: '$578K',
      desc: locale === 'es' ? 'A Better Denver PAC' : 'A Better Denver PAC',
    },
    { name: 'Michael Bloomberg', amount: 500, label: '$500K', desc: '' },
    {
      name: locale === 'es' ? 'Asoc. Nac. de Agentes' : 'Natl. Assoc. of Realtors',
      amount: 150,
      label: '$150K',
      desc: locale === 'es' ? 'A Better Denver PAC' : 'A Better Denver PAC',
    },
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Quién Paga por la Política de Denver (2023)'
          : 'Who Pays for Denver Politics (2023)'}
      </ChartTitle>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart
          data={donors}
      ...
      >
        <span style={{ fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
          ...
          <span style={{ color: C.textMuted }}>
            {locale === 'es' ? 'Multimillonarios' : 'Billionaires'}
          </span>
        </span>
        <span style={{ fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
          <span
          ...
          />
          <span style={{ color: C.textMuted }}>
            {locale === 'es' ? 'Inmobiliario' : 'Real Estate'}
          </span>
        </span>
      </div>
    </div>
...
function FEFvsPACs({ locale }: { locale: string }) {
  const items = [
    {
      ...
      sub:
        locale === 'es'
          ? `Dividido entre ${totalRaised2023.fefCandidates} candidatos`
          : `Split across ${totalRaised2023.fefCandidates} candidates`,
      color: C.primary,
      bg: C.bgAlt,
    },
    {
      ...
      sub:
        locale === 'es'
          ? 'Concentrado en pocos candidatos'
          : 'Concentrated on a handful of candidates',
      color: C.danger,
      bg: '#FDF2F2',
    },
  ];
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Dinero Público vs. Dinero Privado (2023)'
          : 'Public Money vs. Private Money (2023)'}
      </ChartTitle>
      <div style={{ display: 'flex', gap: '1.25rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
        {items.map((item, i) => (
          ...
          >
            ...
            <div
              style={{ fontSize: '0.9rem', color: C.text, fontWeight: 600, marginTop: '0.5rem' }}
            >
              {item.label}
            </div>
        ...
        {councilVotes.map((v, i) => (
          <div
            ...
            style={{
              ...
              background:
                v.position === 'bad' ? '#FDF2F2' : v.position === 'vetoed' ? '#FFFBEB' : C.bgAlt,
              borderRadius: 10,
              borderLeft: `4px solid ${positionColors[v.position]}`,
            }}
          >
            <div
              style={{
                fontSize: '1.1rem',
                fontWeight: 800,
                color: positionColors[v.position],
                fontFamily: 'Inter, system-ui, sans-serif',
                minWidth: 55,
                textAlign: 'center',
              }}
...
function SchoolBoardLesson({ locale }: { locale: string }) {
  const sides = [
    {
      ...
      result:
        locale === 'es' ? schoolBoard2025.proCharter.resultEs : schoolBoard2025.proCharter.result,
      color: C.danger,
      bg: '#FDF2F2',
    },
    {
      label:
        locale === 'es'
          ? 'Sindicato (Students Deserve Better)'
          : 'Union-Backed (Students Deserve Better)',
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Elecciones Escolares 2025: El Dinero No Siempre Gana'
          : "2025 School Board: Money Doesn't Always Win"}
      </ChartTitle>
      <div style={{ display: 'flex', gap: '1.25rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
        {sides.map((s, i) => (
          <div
          ...
          >
            ...
            <div
              style={{ fontSize: '0.85rem', color: C.text, fontWeight: 600, marginTop: '0.5rem' }}
            >
              {s.label}
            </div>
            <div
...
function BallotWars2025({ locale }: { locale: string }) {
  ...
  const sides = [
    ...
    },
    {
      ...
      sub: locale === 'es' ? '90% de la industria tabacalera' : '90% from tobacco industry',
      color: C.danger,
      bg: '#FDF2F2',
    },
  ];
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? 'Guerras Electorales 2025: Referéndum 310'
          : '2025 Ballot Wars: Referendum 310'}
      </ChartTitle>
...
function Outlook2027({ locale }: { locale: string }) {
  ...
  return (
    <div>
      <ChartTitle>
        {locale === 'es'
          ? '2027: Nuevas Reglas y Primeros Candidatos'
          : '2027: New Rules & Early Candidates'}
      </ChartTitle>
      <div
        style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.75rem' }}
      >
        {items.map((item, i) => (
          ...
          >
            ...
            <span style={{ fontSize: '0.9rem', color: C.text, fontWeight: 500 }}>{item.label}</span>
          </div>
        ))}
      </div>
      <div
