...
for (const locale of LOCALES) {
  ...
  if (!existsSync(jsonPath)) {
    warn(
      `Missing UI strings: src/i18n/${locale}.json`,
      `No ${LOCALE_NAMES[locale]} UI translations found. Run the translation workflow to generate them, or English will be used as fallback.`,
    );
    continue;
  }
  ...
  try {
    ...
    if (localeKeyCount < enKeyCount) {
      warn(
        `Incomplete UI strings: src/i18n/${locale}.json (${localeKeyCount}/${enKeyCount} keys)`,
        `${LOCALE_NAMES[locale]} UI translations are missing ${enKeyCount - localeKeyCount} keys. English fallback will be used for missing keys.`,
      );
    } else {
...
for (const locale of LOCALES) {
  ...
  if (!existsSync(policiesDir)) {
    warn(
      `Missing policy translations: src/content/policies-${locale}/`,
      `No ${LOCALE_NAMES[locale]} policy translations found. Run the translation workflow.`,
    );
    continue;
  }
  ...
  if (realFiles.length === 0) {
    warn(
      `Empty policy translations: src/content/policies-${locale}/`,
      `${LOCALE_NAMES[locale]} policy directory exists but has no translations.`,
    );
    continue;
  }
...
for (const locale of LOCALES) {
  ...
  if (!existsSync(fmPath)) {
    warn(
      `Missing policy metadata: src/i18n/policy-meta-${locale}.json`,
      `No ${LOCALE_NAMES[locale]} policy titles/summaries found.`,
    );
    continue;
  }
  ...
  try {
    ...
    if (fmCount < enPolicies.length) {
      warn(
        `Incomplete policy metadata: src/i18n/policy-meta-${locale}.json (${fmCount}/${enPolicies.length})`,
        `${LOCALE_NAMES[locale]} is missing metadata for ${enPolicies.length - fmCount} policies.`,
      );
    } else {
