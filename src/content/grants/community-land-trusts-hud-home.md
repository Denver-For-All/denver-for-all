...
5. **Advances fair housing goals:** Targets neighborhoods with documented histories of redlining and racial displacement, creating permanently affordable homeownership opportunities for communities of color
...
1. **Confirm current HOME allocation and set-aside availability** through Denver HOST and the [HUD Exchange](https://www.hudexchange.info/programs/home/)
2. **Update displacement and housing cost data** using the latest HOST annual reports, American Community Survey data, and Elevation CLT portfolio numbers
3. **Adjust budget** based on current property acquisition costs in target neighborhoods (Denver MLS data, HOST acquisition records)
4. **Add required federal forms** (SF-424, HUD-2880, certifications, CHDO qualification documentation if applicable, environmental review records)
5. **Coordinate with Denver HOST HOME program staff** for submission, match documentation, and compliance planning
6. **Engage Elevation CLT leadership** to confirm organizational capacity, pipeline projections, and subrecipient agreement terms
